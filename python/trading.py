...
class NuBot(ConnectionThread):
  def __init__(self, conn, key, secret, exchange, unit, logger = None):
    ...
    if unit != 'usd':
      if unit == 'btc':
        ...
        if self.logger: self.logger.error('no price feed available for %s', unit)
      self.options['secondary-peg-options']['wallshift-threshold'] = 0.3
      self.options['secondary-peg-options']['spread'] = 0.0

  def run(self):
    if self.logger: self.logger.info("starting NuBot for unit %s on exchange %s", self.unit, repr(self.exchange))
  ...
  def shutdown(self):
    if self.process:
      if self.logger: self.logger.info("stopping NuBot for unit %s on exchange %s", self.unit, repr(self.exchange))
...
class PyBot(ConnectionThread):
  ...
  def shutdown(self):
    if self.logger: self.logger.info("stopping PyBot for unit %s on exchange %s", self.unit, repr(self.exchange))
    trials = 0
    while trials < 10:
      try:
      ...
      if 'error' in response:
        if self.logger: self.logger.error('unable to cancel orders for unit %s on exchange %s (trial %d): %s', self.unit, repr(self.exchange), trials + 1, response['error'])
      else:
        if self.logger: self.logger.info('successfully deleted all orders for unit %s on exchange %s', self.unit, repr(self.exchange))
        break
      trials = trials + 1
  ...
  def place(self, side):
    ...
    if 'error' in response:
      if self.logger: self.logger.error('unable to receive balance for unit %s on exchange %s: %s', exunit, repr(self.exchange), response['error'])
  ...
  def reset(self, cancel = True):
    ...
    while 'error' in response:
      ...
      if cancel:
        ...
        if 'error' in response:
          if self.logger: self.logger.error('unable to cancel orders for unit %s on exchange %s: %s', self.unit, repr(self.exchange), response['error'])
        else:
          if self.logger: self.logger.info('successfully deleted all orders for unit %s on exchange %s', self.unit, repr(self.exchange))
      ...
      if 'error' in response:
        self.exchange.adjust(response['error'])
        if self.logger: self.logger.info('trying to adjust nonce of exchange %s to %d', repr(self.exchange), self.exchange._shift)
    self.release_lock()

  def run(self):
    if self.logger: self.logger.info("starting PyBot for unit %s on exchange %s", self.unit, repr(self.exchange))
    self.update_price()
    self.update_interest()
